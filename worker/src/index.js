// SaveCents OTP Worker (M5.32)
// Free Cloudflare Worker + Brevo free tier = branded OTP emails, no Blaze.
//
// Endpoints:
//   POST /send-otp   { email }         -> generates a 6-digit code, stores a
//                                         SHA-256 hash in KV (10 min TTL,
//                                         5 attempts), emails it via Brevo.
//                                         Rate limit: 3 sends/hour per email.
//   POST /verify-otp { email, code }   -> { ok, reason? }
//
// Setup (once, on the owner's PC - see worker/README.md):
//   wrangler kv namespace create OTP_KV   -> paste the id into wrangler.toml
//   wrangler secret put BREVO_API_KEY     -> paste the Brevo v3 API key
//   wrangler deploy                       -> paste the URL into src/services/otp.ts

const CODE_TTL_SECONDS = 600;
const MAX_ATTEMPTS = 5;
const SENDS_PER_HOUR = 3;

const CORS = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Methods': 'POST, OPTIONS',
  'Access-Control-Allow-Headers': 'Content-Type',
};

const json = (obj, status = 200) =>
  new Response(JSON.stringify(obj), {
    status,
    headers: { 'Content-Type': 'application/json', ...CORS },
  });

async function sha256(text) {
  const data = new TextEncoder().encode(text);
  const hash = await crypto.subtle.digest('SHA-256', data);
  return [...new Uint8Array(hash)].map((b) => b.toString(16).padStart(2, '0')).join('');
}

const validEmail = (e) => typeof e === 'string' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e) && e.length < 200;

// Mint a Google access token from the Firebase service account (secret
// FIREBASE_SA_JSON = the whole downloaded JSON, pasted as one line). Cached
// until near expiry. Powers the no-link password reset.
let tokenCache = { token: null, exp: 0 };
async function getAccessToken(env) {
  const now = Math.floor(Date.now() / 1000);
  if (tokenCache.token && now < tokenCache.exp - 60) return tokenCache.token;
  const sa = JSON.parse(env.FIREBASE_SA_JSON);
  const b64u = (str) => btoa(str).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
  const header = b64u(JSON.stringify({ alg: 'RS256', typ: 'JWT' }));
  const claims = b64u(JSON.stringify({
    iss: sa.client_email,
    scope: 'https://www.googleapis.com/auth/identitytoolkit',
    aud: 'https://oauth2.googleapis.com/token',
    iat: now,
    exp: now + 3600,
  }));
  const unsigned = `${header}.${claims}`;
  const pem = sa.private_key.replace(/-----[^-]+-----/g, '').replace(/\s+/g, '');
  const keyBytes = Uint8Array.from(atob(pem), (c) => c.charCodeAt(0));
  const key = await crypto.subtle.importKey(
    'pkcs8', keyBytes.buffer,
    { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['sign'],
  );
  const sig = new Uint8Array(await crypto.subtle.sign('RSASSA-PKCS1-v1_5', key, new TextEncoder().encode(unsigned)));
  let sigStr = '';
  for (const b of sig) sigStr += String.fromCharCode(b);
  const jwt = `${unsigned}.${b64u(sigStr)}`;
  const res = await fetch('https://oauth2.googleapis.com/token', {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: `grant_type=${encodeURIComponent('urn:ietf:params:oauth:grant-type:jwt-bearer')}&assertion=${jwt}`,
  });
  const data = await res.json();
  if (!data.access_token) throw new Error('token mint failed');
  tokenCache = { token: data.access_token, exp: now + (data.expires_in ?? 3600) };
  return tokenCache.token;
}

// v5.11: every SaveCents email wears the app's editorial theme - warm
// paper background, white card with a hairline border, forest green
// accents, charcoal ink. Shared shell for OTP and report emails.
function emailShell(senderName, inner, logoUrl) {
  const brand = logoUrl
    ? `<img src="${logoUrl}" width="164" style="display:block;border:0;margin:0 auto;" alt="${senderName}" />`
    : `<span style="display:inline-block;background:#8A5C00;color:#FFFFFF;font-family:Arial,Helvetica,sans-serif;font-weight:800;font-size:13px;letter-spacing:0.4px;border-radius:999px;padding:8px 16px;">&#162; ${senderName}</span>`;
  return `<!DOCTYPE html>
<html><body style="margin:0;padding:0;background:#FBF8F2;">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#FBF8F2;padding:32px 12px;">
<tr><td align="center">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:460px;background:#FFFFFF;border:1px solid #E9ECEF;border-radius:20px;overflow:hidden;">
    <tr><td align="center" style="padding:30px 30px 0;">
      ${brand}
    </td></tr>
    <tr><td style="padding:18px 30px 0;"><div style="height:1px;background:#E9ECEF;"></div></td></tr>
    ${inner}
  </table>
  <div style="font-family:Arial,Helvetica,sans-serif;color:#ADB5BD;font-size:11px;padding-top:16px;">Sent by ${senderName} &middot; Manila, Philippines</div>
</td></tr>
</table>
</body></html>`;
}

function emailHtml(code, senderName, title, sub, logoUrl) {
  const digits = String(code)
    .split('')
    .map(
      (d) =>
        `<td style="width:44px;height:56px;background:#FAF3E0;border:1px solid #E9DCB0;border-radius:12px;text-align:center;font-size:26px;font-weight:800;color:#8A5C00;font-family:'SF Mono',Menlo,Consolas,monospace;">${d}</td><td style="width:8px;"></td>`,
    )
    .join('');
  return emailShell(senderName, `
    <tr><td style="padding:24px 30px 8px;font-family:Arial,Helvetica,sans-serif;">
      <div style="color:#1A1D20;font-size:22px;font-weight:800;line-height:1.3;">${title}</div>
      <div style="color:#6C757D;font-size:14px;line-height:1.6;margin-top:8px;">${sub}</div>
    </td></tr>
    <tr><td style="padding:18px 30px 6px;">
      <table role="presentation" cellpadding="0" cellspacing="0"><tr>${digits}</tr></table>
    </td></tr>
    <tr><td style="padding:16px 30px 28px;font-family:Arial,Helvetica,sans-serif;">
      <div style="color:#ADB5BD;font-size:12px;line-height:1.6;">Didn't request this? You can safely ignore this email - your account stays untouched. Never share this code with anyone; ${senderName} will never ask for it.</div>
    </td></tr>`, logoUrl);
}

// Planner v3.2b: branded split emails. Full SaveCents shell (logo, palette,
// share box), copy kept strictly transactional: no taglines, no buttons, no
// promo wording. The multipart textContent stays for deliverability.
function splitShareHtml(senderName, p, logoUrl) {
  return emailShell(senderName, `
    <tr><td style="padding:24px 30px 8px;font-family:Arial,Helvetica,sans-serif;">
      <div style="color:#1A1D20;font-size:21px;font-weight:800;line-height:1.3;">Your share of ${p.title}</div>
      <div style="color:#6C757D;font-size:14px;line-height:1.6;margin-top:8px;">Hi ${p.name}, ${p.payerName} paid this one and ${p.userName} split it ${p.even === false ? 'by what each person got' : `${p.headcount} ways`}.</div>
    </td></tr>
    <tr><td style="padding:14px 30px 4px;">
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#FAF3E0;border:1px solid #E9DCB0;border-radius:14px;">
        <tr><td style="padding:16px 18px;font-family:Arial,Helvetica,sans-serif;">
          <div style="color:#6C757D;font-size:13px;line-height:1.7;">${p.even === false ? `${p.totalFmt} total, custom shares` : `${p.totalFmt} &divide; ${p.headcount} people`}</div>
          <div style="color:#8A5C00;font-size:26px;font-weight:800;margin-top:4px;">${p.shareFmt}</div>
          <div style="color:#6C757D;font-size:12px;margin-top:2px;">your share</div>
        </td></tr>
      </table>
    </td></tr>
    <tr><td style="padding:16px 30px 28px;font-family:Arial,Helvetica,sans-serif;">
      <div style="color:#1A1D20;font-size:14px;line-height:1.7;">Once you've paid, just message ${p.payerName} to let them know.</div>
    </td></tr>`, logoUrl);
}

function splitManageHtml(senderName, r, link, logoUrl) {
  return emailShell(senderName, `
    <tr><td style="padding:24px 30px 8px;font-family:Arial,Helvetica,sans-serif;">
      <div style="color:#1A1D20;font-size:21px;font-weight:800;line-height:1.3;">You covered ${r.title}</div>
      <div style="color:#6C757D;font-size:14px;line-height:1.6;margin-top:8px;">${r.even === false ? `${r.userName} split it by what each person got: ${r.totalFmt} total.` : `${r.userName} split it ${r.headcount} ways: ${r.totalFmt} total, ${r.shareFmt} each.`}</div>
    </td></tr>
    <tr><td style="padding:14px 30px 4px;font-family:Arial,Helvetica,sans-serif;">
      <div style="color:#1A1D20;font-size:14px;line-height:1.7;">This link was generated by ${r.userName}. You can manage the bill splitting here and tick off who already paid you.</div>
    </td></tr>
    <tr><td align="center" style="padding:18px 30px 6px;">
      <a href="${link}" style="display:inline-block;background:#8A5C00;color:#FFFFFF;font-family:Arial,Helvetica,sans-serif;font-weight:800;font-size:15px;text-decoration:none;border-radius:14px;padding:14px 34px;">Manage the split</a>
    </td></tr>
    <tr><td style="padding:12px 30px 28px;font-family:Arial,Helvetica,sans-serif;" align="center">
      <div style="color:#ADB5BD;font-size:12px;line-height:1.6;">The link expires in 15 days.</div>
    </td></tr>`, logoUrl);
}

// Planner v4: the lend reminder. Same shell, amount box, strictly neutral
// copy. This never carries pressure language, links, or third parties.
function lendEmailHtml(senderName, p, logoUrl) {
  return emailShell(senderName, `
    <tr><td style="padding:24px 30px 8px;font-family:Arial,Helvetica,sans-serif;">
      <div style="color:#1A1D20;font-size:21px;font-weight:800;line-height:1.3;">A friendly reminder</div>
      <div style="color:#6C757D;font-size:14px;line-height:1.6;margin-top:8px;">Hi ${p.name}, just a reminder about the amount ${p.userName} lent you.</div>
    </td></tr>
    <tr><td style="padding:14px 30px 4px;">
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#FAF3E0;border:1px solid #E9DCB0;border-radius:14px;">
        <tr><td style="padding:16px 18px;font-family:Arial,Helvetica,sans-serif;">
          <div style="color:#8A5C00;font-size:26px;font-weight:800;">${p.amountFmt}</div>
          <div style="color:#6C757D;font-size:13px;margin-top:2px;">due ${p.dueLabel}</div>
        </td></tr>
      </table>
    </td></tr>
    <tr><td style="padding:16px 30px 28px;font-family:Arial,Helvetica,sans-serif;">
      <div style="color:#1A1D20;font-size:14px;line-height:1.7;">Once you've sent it, let ${p.userName} know directly.</div>
      <div style="color:#ADB5BD;font-size:12px;line-height:1.6;margin-top:14px;">You're receiving this because ${p.userName} recorded this loan with your email and your agreement.</div>
    </td></tr>`, logoUrl);
}

// Planner v3.2: minimal personal-looking html. One font stack, no images,
// no colors beyond text. Paragraph breaks only.
function plainHtml(text) {
  const esc = text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const paras = esc.split('\n\n').map((pp) => `<p style="margin:0 0 14px;">${pp.replace(/\n/g, '<br>')}</p>`).join('');
  return `<div style="font-family:-apple-system,Segoe UI,Arial,sans-serif;font-size:15px;line-height:1.6;color:#1A1D20;">${paras}</div>`;
}

// Hosted split bill: KV record sb:{token} =
// { title, totalFmt, shareFmt, headcount, payerName, userName,
//   people: [{id,name,paid}], my: {included, label, paid} }
const SPLIT_TTL = 15 * 24 * 3600; // 15 days, then the link retires itself

async function handleSplitRoutes(request, env, url) {
  const seg = url.pathname.split('/').filter(Boolean); // e.g. ['split','abc']
  const esc = (v) => String(v ?? '').replace(/[<>&"]/g, '');

  // POST /split-create : store the bill, email the payer their manage link.
  if (url.pathname === '/split-create' && request.method === 'POST') {
    let body;
    try { body = await request.json(); } catch { return json({ ok: false, reason: 'Bad request' }, 400); }
    const clean = (v, n) => String(v ?? '').replace(/[<>]/g, '').trim().slice(0, n);
    const record = {
      title: clean(body.title, 80) || 'a shared bill',
      totalFmt: clean(body.totalFmt, 30),
      shareFmt: clean(body.shareFmt, 30),
      headcount: Math.min(Math.max(parseInt(body.headcount, 10) || 2, 2), 50),
      payerName: clean(body.payerName, 60) || 'the payer',
      userName: clean(body.userName, 60) || 'A SaveCents user',
      people: (Array.isArray(body.people) ? body.people : []).slice(0, 20).map((pp) => ({
        id: clean(pp.id, 40), name: clean(pp.name, 60) || 'Someone', paid: false,
        // Planner v5: per-person share on custom bills. Absent (older app
        // versions) falls back to the bill-level shareFmt at render time.
        shareFmt: clean(pp.shareFmt, 30) || undefined,
      })),
      my: {
        included: !!body.includeUser, label: clean(body.userLabel, 60) || 'the SaveCents user', paid: false,
        shareFmt: clean(body.userShareFmt, 30) || undefined,
      },
      // false = custom amounts; anything else (including absent) = even.
      even: body.even !== false,
    };
    if (!record.totalFmt || !record.shareFmt) return json({ ok: false, reason: 'Bad request' }, 400);
    const token = token24();
    await env.OTP_KV.put(`sb:${token}`, JSON.stringify(record), { expirationTtl: SPLIT_TTL });
    const link = `${url.protocol}//${url.host}/split/${token}`;

    const payerEmail = String(body.payerEmail ?? '').trim().toLowerCase();
    let emailed = false;
    if (validEmail(payerEmail)) {
      const mathBit = record.even === false
        ? `${record.totalFmt}, split by what each person got`
        : `${record.totalFmt}, split ${record.headcount} ways, ${record.shareFmt} each`;
      const text = `Hi ${record.payerName},\n\nYou covered ${record.title} (${mathBit}).\n\nThis link was generated by ${record.userName}. You can manage the bill splitting here and tick off who already paid you:\n\n${link}\n\nThe link expires in 15 days.\n\n${record.userName}, via ${env.SENDER_NAME}`;
      const res = await fetch('https://api.brevo.com/v3/smtp/email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'api-key': env.BREVO_API_KEY },
        body: JSON.stringify({
          sender: { name: `${record.userName} via ${env.SENDER_NAME}`, email: env.SENDER_EMAIL },
          replyTo: { email: env.REPLY_TO_EMAIL || 'support@savecents.app' },
          to: [{ email: payerEmail }],
          subject: `You covered ${record.title}, here's your tracker`,
          textContent: text,
          htmlContent: splitManageHtml(env.SENDER_NAME, record, link, `${url.protocol}//${url.host}/logo-v2.png`),
        }),
      });
            emailed = res.ok;
      if (!res.ok) console.log('split manage email failed', res.status);
    }
    return json({ ok: true, token, url: link, emailed });
  }

  // GET /split-state/:token : the app pulls the payer's ticks.
  if (seg[0] === 'split-state' && seg[1] && request.method === 'GET') {
    const raw = await env.OTP_KV.get(`sb:${seg[1]}`);
    if (!raw) return json({ ok: false, reason: 'gone' }, 404);
    const r = JSON.parse(raw);
    return json({ ok: true, people: r.people.map((pp) => ({ id: pp.id, paid: pp.paid })), myPaid: !!r.my?.paid });
  }

  // POST /split-set/:token : the app pushes a tick (its own share or a
  // person it marked), keeping both sides matched.
  if (seg[0] === 'split-set' && seg[1] && request.method === 'POST') {
    let body;
    try { body = await request.json(); } catch { return json({ ok: false, reason: 'Bad request' }, 400); }
    const raw = await env.OTP_KV.get(`sb:${seg[1]}`);
    if (!raw) return json({ ok: false, reason: 'gone' }, 404);
    const r = JSON.parse(raw);
    if (body.mine !== undefined) r.my.paid = !!body.mine;
    if (body.pid) r.people = r.people.map((pp) => (pp.id === body.pid ? { ...pp, paid: !!body.paid } : pp));
    await env.OTP_KV.put(`sb:${seg[1]}`, JSON.stringify(r), { expirationTtl: SPLIT_TTL });
    return json({ ok: true });
  }

  // The manage page itself. GET renders, form POST toggles then bounces back.
  if (seg[0] === 'split' && seg[1]) {
    const key = `sb:${seg[1]}`;
    const raw = await env.OTP_KV.get(key);
    if (!raw) {
      return new Response('<meta name="viewport" content="width=device-width,initial-scale=1"><body style="font-family:-apple-system,Segoe UI,Arial,sans-serif;padding:40px 24px;color:#1A1D20;"><h3>This link has expired</h3><p>Split links last 15 days. Ask for a fresh one.</p></body>', { status: 404, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
    }
    const r = JSON.parse(raw);

    if (request.method === 'POST') {
      const form = await request.formData();
      const pid = form.get('pid');
      const mine = form.get('mine');
      if (mine !== null) r.my.paid = !r.my.paid;
      else if (pid) r.people = r.people.map((pp) => (pp.id === pid ? { ...pp, paid: !pp.paid } : pp));
      await env.OTP_KV.put(key, JSON.stringify(r), { expirationTtl: SPLIT_TTL });
      return new Response(null, { status: 303, headers: { Location: url.pathname } });
    }

    const row = (label, paid, hidden, shareFmt) => `
      <form method="post" style="margin:0;">${hidden}
        <button type="submit" style="width:100%;display:flex;align-items:center;gap:12px;text-align:left;background:${paid ? '#FAF3E0' : '#fff'};border:1px solid ${paid ? '#E9DCB0' : '#E3E6E8'};border-radius:14px;padding:14px 16px;margin-bottom:10px;font-size:16px;font-family:inherit;color:#1A1D20;">
          <span style="display:inline-flex;width:22px;height:22px;">${paid
            ? '<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" fill="#8A5C00"/><path d="M8 12.5l2.6 2.6L16 9.7" stroke="#FFFFFF" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>'
            : '<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="#ADB5BD" stroke-width="2"/></svg>'}</span>
          <span style="flex:1;${paid ? 'text-decoration:line-through;color:#6C757D;' : ''}">${esc(label)}</span>
          <span style="font-weight:700;color:${paid ? '#6C757D' : '#8A5C00'};">${esc(shareFmt || r.shareFmt)}</span>
        </button>
      </form>`;
    const doneCount = r.people.filter((pp) => pp.paid).length + (r.my.included && r.my.paid ? 1 : 0);
    const totalCount = r.people.length + (r.my.included ? 1 : 0);
    const mathLine = r.even === false
      ? `${esc(r.totalFmt)} total \u00b7 <b style="color:#8A5C00;">custom shares</b> \u00b7 ${doneCount}/${totalCount} settled`
      : `${esc(r.totalFmt)} \u00f7 ${r.headcount} = <b style="color:#8A5C00;">${esc(r.shareFmt)} each</b> \u00b7 ${doneCount}/${totalCount} settled`;
    const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(r.title)}</title></head>
<body style="font-family:-apple-system,Segoe UI,Arial,sans-serif;background:#FBF8F2;color:#1A1D20;margin:0;padding:28px 20px;">
  <div style="max-width:440px;margin:0 auto;">
    <div style="font-size:13px;color:#6C757D;">You covered this one</div>
    <h2 style="margin:4px 0 6px;font-size:24px;">${esc(r.title)}</h2>
    <div style="color:#6C757D;font-size:14px;margin-bottom:18px;">${mathLine}</div>
    ${r.my.included ? row(r.my.label, r.my.paid, '<input type="hidden" name="mine" value="1">', r.my.shareFmt) : ''}
    ${r.people.map((pp) => row(pp.name, pp.paid, `<input type="hidden" name="pid" value="${esc(pp.id)}">`, pp.shareFmt)).join('')}
    <div style="color:#ADB5BD;font-size:12px;margin-top:16px;line-height:1.6;">This link was generated by ${esc(r.userName)}. Tap a name to tick off who already paid you. The link expires in 15 days.</div>
  </div>
</body></html>`;
    return new Response(html, { headers: { 'Content-Type': 'text/html; charset=utf-8', ...CORS } });
  }

  return json({ ok: false, reason: 'not-found' }, 404);
}

function reportEmailHtml(senderName, monthLabel, preparedFor, stats, links, logoUrl) {
  const fmt = (n) => '\u20B1' + Number(n).toLocaleString('en-PH', { maximumFractionDigits: 2 });
  const statCell = (label, value, color) => `
    <td width="33%" style="padding:12px 14px;background:#FBF8F2;border:1px solid #E9ECEF;border-radius:12px;font-family:Arial,Helvetica,sans-serif;">
      <div style="color:#6C757D;font-size:10px;letter-spacing:1px;font-weight:700;">${label}</div>
      <div style="color:${color};font-size:18px;font-weight:800;margin-top:2px;">${value}</div>
    </td>`;
  const btn = (href, label, solid) => `
    <a href="${href}" style="display:inline-block;font-family:Arial,Helvetica,sans-serif;font-weight:800;font-size:14px;text-decoration:none;border-radius:12px;padding:13px 22px;margin-right:10px;${solid
      ? 'background:#8A5C00;color:#FFFFFF;'
      : 'background:#FFFFFF;color:#8A5C00;border:1px solid #E9DCB0;'}">${label}</a>`;
  return emailShell(senderName, `
    <tr><td style="padding:24px 30px 6px;font-family:Arial,Helvetica,sans-serif;">
      <div style="color:#1A1D20;font-size:22px;font-weight:800;line-height:1.3;">Your ${monthLabel} report</div>
      <div style="color:#6C757D;font-size:14px;line-height:1.6;margin-top:8px;">Hi ${preparedFor}, Cents put together your money month: ${stats.count} transaction${stats.count === 1 ? '' : 's'} from the 1st to the last day of ${monthLabel}.</div>
    </td></tr>
    <tr><td style="padding:14px 30px 4px;">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="8"><tr>
        ${statCell('INCOME', fmt(stats.income), '#8A5C00')}
        ${statCell('EXPENSES', fmt(stats.expenses), '#1A1D20')}
        ${statCell('NET', (stats.net < 0 ? '-' : '') + fmt(Math.abs(stats.net)), stats.net < 0 ? '#DC2626' : '#8A5C00')}
      </tr></table>
    </td></tr>
    <tr><td style="padding:18px 30px 8px;font-family:Arial,Helvetica,sans-serif;">
      ${btn(links.pdf, 'Download PDF', true)}
      ${btn(links.csv, 'Download CSV', false)}
    </td></tr>
    <tr><td style="padding:8px 30px 28px;font-family:Arial,Helvetica,sans-serif;">
      <div style="color:#ADB5BD;font-size:12px;line-height:1.6;">Both files are also attached to this email. The download buttons work for 7 days.</div>
    </td></tr>`, logoUrl);
}

async function sendViaBrevo(env, toEmail, code, kind, logoUrl) {
  const title = kind === 'reset' ? 'Reset your password' : 'Your verification code';
  const sub = kind === 'reset'
    ? `Enter this code in the ${env.SENDER_NAME} app to set a new password. It works for the next 10 minutes.`
    : `Enter this code in the ${env.SENDER_NAME} app to continue. It works for the next 10 minutes.`;
  const res = await fetch('https://api.brevo.com/v3/smtp/email', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'api-key': env.BREVO_API_KEY,
    },
    body: JSON.stringify({
      sender: { name: env.SENDER_NAME, email: env.SENDER_EMAIL },
          replyTo: { email: env.REPLY_TO_EMAIL || 'support@savecents.app' },
      to: [{ email: toEmail }],
      subject: kind === 'reset'
        ? `${code} is your ${env.SENDER_NAME} password reset code`
        : `${code} is your ${env.SENDER_NAME} code`,
      htmlContent: emailHtml(code, env.SENDER_NAME, title, sub, logoUrl),
    }),
  });
  if (!res.ok) {
    const body = await res.text();
    throw new Error(`Brevo ${res.status}: ${body.slice(0, 200)}`);
  }
}

// v5.12: the SaveCents wordmark, served at /logo.png for the emails
// (remote images render everywhere; data URIs get stripped by Gmail).
const LOGO_B64 = '';

const token24 = () => [...crypto.getRandomValues(new Uint8Array(12))].map((b) => b.toString(16).padStart(2, '0')).join('');

export default {
  async fetch(request, env) {
    if (request.method === 'OPTIONS') return new Response(null, { headers: CORS });

    const url = new URL(request.url);

    // v2 path busts Gmail's image-proxy cache of the old (black) render.
    if (request.method === 'GET' && (url.pathname === '/logo.png' || url.pathname === '/logo-v2.png')) {
      const bytes = Uint8Array.from(atob(LOGO_B64), (c) => c.charCodeAt(0));
      return new Response(bytes, {
        headers: { 'Content-Type': 'image/png', 'Cache-Control': 'public, max-age=604800', ...CORS },
      });
    }

    // Report downloads: the email buttons hit this with a one-time token.
    if (request.method === 'GET' && url.pathname.startsWith('/report/')) {
      const token = url.pathname.split('/')[2] ?? '';
      const raw = token ? await env.OTP_KV.get(`rep:${token}`) : null;
      if (!raw) return new Response('This download link has expired.', { status: 404, headers: { 'Content-Type': 'text/plain', ...CORS } });
      const file = JSON.parse(raw);
      const bytes = Uint8Array.from(atob(file.data), (c) => c.charCodeAt(0));
      return new Response(bytes, {
        headers: {
          'Content-Type': file.mime,
          'Content-Disposition': `attachment; filename="${file.name}"`,
          ...CORS,
        },
      });
    }

    // Planner v3.2: hosted split manage page + state API. These routes parse
    // their own input and skip the shared email check (a GET page and form
    // posts have no body.email).
    if (url.pathname.startsWith('/split')) {
      return handleSplitRoutes(request, env, url);
    }

    if (request.method !== 'POST') return json({ ok: false, reason: 'POST only' }, 405);
    let body;
    try {
      body = await request.json();
    } catch {
      return json({ ok: false, reason: 'Bad request' }, 400);
    }
    const email = String(body.email ?? '').trim().toLowerCase();
    if (!validEmail(email)) return json({ ok: false, reason: 'Bad email' }, 400);

    if (url.pathname === '/send-otp') {
      // Per-email rate limit: SENDS_PER_HOUR.
      const rlKey = `rl:${email}`;
      const sends = parseInt((await env.OTP_KV.get(rlKey)) ?? '0', 10);
      if (sends >= SENDS_PER_HOUR) return json({ ok: false, reason: 'rate' }, 429);
      await env.OTP_KV.put(rlKey, String(sends + 1), { expirationTtl: 3600 });

      const code = String(Math.floor(100000 + Math.random() * 900000));
      const record = { hash: await sha256(`${email}:${code}`), attemptsLeft: MAX_ATTEMPTS };
      await env.OTP_KV.put(`otp:${email}`, JSON.stringify(record), { expirationTtl: CODE_TTL_SECONDS });

      try {
        await sendViaBrevo(env, email, code, 'verify', `${url.protocol}//${url.host}/logo-v2.png`);
      } catch (e) {
        await env.OTP_KV.delete(`otp:${email}`);
        console.log('send failed', String(e));
        return json({ ok: false, reason: 'send-failed' }, 502);
      }
      return json({ ok: true });
    }

    // Planner v3.2: one person's share, written like a person wrote it.
    // Plain multipart, no images, no boxes, sender is the user via the app.
    // That shape is what keeps it in Primary instead of Promotions.
    if (url.pathname === '/send-split') {
      const rlKey = `rlsplit:${email}`;
      const sends = parseInt((await env.OTP_KV.get(rlKey)) ?? '0', 10);
      if (sends >= 6) return json({ ok: false, reason: 'rate' }, 429);
      await env.OTP_KV.put(rlKey, String(sends + 1), { expirationTtl: 3600 });

      const clean = (v, n) => String(v ?? '').replace(/[<>]/g, '').trim().slice(0, n);
      const name = clean(body.name, 60) || 'there';
      const userName = clean(body.userName, 60) || 'A SaveCents user';
      const payerName = clean(body.payerName, 60) || userName;
      const title = clean(body.title, 80) || 'a shared bill';
      const totalFmt = clean(body.totalFmt, 30);
      const shareFmt = clean(body.shareFmt, 30);
      const headcount = Math.min(Math.max(parseInt(body.headcount, 10) || 2, 2), 50);
      // Planner v5: false = custom amounts (never claim an equal division).
      const even = body.even !== false;
      if (!totalFmt || !shareFmt) return json({ ok: false, reason: 'Bad request' }, 400);

      const mathBit = even
        ? `${totalFmt}, split ${headcount} ways`
        : `${totalFmt}, split by what each person got`;
      const text = `Hi ${name},\n\n${payerName} paid ${title} (${mathBit}). Your share is ${shareFmt}.\n\nOnce you've paid, just message ${payerName} to let them know.\n\n${userName}, via ${env.SENDER_NAME}`;
      const res = await fetch('https://api.brevo.com/v3/smtp/email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'api-key': env.BREVO_API_KEY },
        body: JSON.stringify({
          sender: { name: `${userName} via ${env.SENDER_NAME}`, email: env.SENDER_EMAIL },
          to: [{ email }],
          subject: `${payerName} paid ${title}, your share is ${shareFmt}`,
          textContent: text,
          htmlContent: splitShareHtml(env.SENDER_NAME, { name, userName, payerName, title, totalFmt, shareFmt, headcount, even }, `${url.protocol}//${url.host}/logo-v2.png`),
        }),
      });
      if (!res.ok) {
        const t = await res.text();
        console.log('split send failed', res.status, t.slice(0, 200));
        return json({ ok: false, reason: 'send-failed' }, 502);
      }
      return json({ ok: true });
    }

    // Planner v4: polite lend payback reminder to the BORROWER only. Only
    // sent when the user confirmed consent in the app, capped at the 7-3-1
    // stages plus manual nudges, neutral tone, nothing clickable.
    if (url.pathname === '/send-lend') {
      const rlKey = `rllend:${email}`;
      const sends = parseInt((await env.OTP_KV.get(rlKey)) ?? '0', 10);
      if (sends >= 5) return json({ ok: false, reason: 'rate' }, 429);
      await env.OTP_KV.put(rlKey, String(sends + 1), { expirationTtl: 24 * 3600 });

      const clean = (v, n) => String(v ?? '').replace(/[<>]/g, '').trim().slice(0, n);
      const name = clean(body.name, 60) || 'there';
      const userName = clean(body.userName, 60) || 'A SaveCents user';
      const amountFmt = clean(body.amountFmt, 30);
      const dueLabel = clean(body.dueLabel, 40);
      if (!amountFmt || !dueLabel) return json({ ok: false, reason: 'Bad request' }, 400);

      const text = `Hi ${name},\n\nJust a friendly reminder about the ${amountFmt} ${userName} lent you, due ${dueLabel}.\n\nOnce you've sent it, let ${userName} know directly.\n\n${userName}, via ${env.SENDER_NAME}`;
      const res = await fetch('https://api.brevo.com/v3/smtp/email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'api-key': env.BREVO_API_KEY },
        body: JSON.stringify({
          sender: { name: `${userName} via ${env.SENDER_NAME}`, email: env.SENDER_EMAIL },
          to: [{ email }],
          subject: `Friendly reminder: ${amountFmt} due ${dueLabel}`,
          textContent: text,
          htmlContent: lendEmailHtml(env.SENDER_NAME, { name, userName, amountFmt, dueLabel }, `${url.protocol}//${url.host}/logo-v2.png`),
        }),
      });
      if (!res.ok) {
        const t = await res.text();
        console.log('lend send failed', res.status, t.slice(0, 200));
        return json({ ok: false, reason: 'send-failed' }, 502);
      }
      return json({ ok: true });
    }

    // v5.11: Cents monthly report. Stores both files in KV for 7 days (the
    // email buttons) AND attaches them to the email itself.
    if (url.pathname === '/send-report') {
      const rlKey = `rlrep:${email}`;
      const sends = parseInt((await env.OTP_KV.get(rlKey)) ?? '0', 10);
      if (sends >= 6) return json({ ok: false, reason: 'rate' }, 429);
      await env.OTP_KV.put(rlKey, String(sends + 1), { expirationTtl: 3600 });

      const monthLabel = String(body.monthLabel ?? '').slice(0, 40);
      const preparedFor = String(body.preparedFor ?? 'there').slice(0, 60);
      const fileBase = String(body.fileBase ?? 'SAVECENTS-REPORT').replace(/[^A-Z0-9-]/gi, '').slice(0, 60) || 'SAVECENTS-REPORT';
      const stats = body.stats ?? { income: 0, expenses: 0, net: 0, count: 0 };
      const csvB64 = String(body.csvBase64 ?? '');
      const pdfB64 = String(body.pdfBase64 ?? '');
      if (!monthLabel || !csvB64 || !pdfB64) return json({ ok: false, reason: 'Bad request' }, 400);
      if (csvB64.length > 4_000_000 || pdfB64.length > 12_000_000) return json({ ok: false, reason: 'too-big' }, 413);

      const SEVEN_DAYS = 7 * 24 * 3600;
      const csvTok = token24();
      const pdfTok = token24();
      await env.OTP_KV.put(`rep:${csvTok}`, JSON.stringify({ mime: 'text/csv', name: `${fileBase}.csv`, data: csvB64 }), { expirationTtl: SEVEN_DAYS });
      await env.OTP_KV.put(`rep:${pdfTok}`, JSON.stringify({ mime: 'application/pdf', name: `${fileBase}.pdf`, data: pdfB64 }), { expirationTtl: SEVEN_DAYS });
      const origin = `${url.protocol}//${url.host}`;
      const links = { csv: `${origin}/report/${csvTok}`, pdf: `${origin}/report/${pdfTok}` };

      const res = await fetch('https://api.brevo.com/v3/smtp/email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'api-key': env.BREVO_API_KEY },
        body: JSON.stringify({
          sender: { name: env.SENDER_NAME, email: env.SENDER_EMAIL },
          replyTo: { email: env.REPLY_TO_EMAIL || 'support@savecents.app' },
          to: [{ email }],
          subject: `Your ${monthLabel} report from ${env.SENDER_NAME}`,
          htmlContent: reportEmailHtml(env.SENDER_NAME, monthLabel, preparedFor, stats, links, `${origin}/logo-v2.png`),
          attachment: [
            { name: `${fileBase}.pdf`, content: pdfB64 },
            { name: `${fileBase}.csv`, content: csvB64 },
          ],
        }),
      });
      if (!res.ok) {
        const t = await res.text();
        console.log('report send failed', res.status, t.slice(0, 200));
        return json({ ok: false, reason: 'send-failed' }, 502);
      }
      return json({ ok: true });
    }

    if (url.pathname === '/reset-request') {
      // No-link password reset: Firebase mints the real oobCode (admin), the
      // user only ever sees a friendly 6-digit code.
      if (!env.FIREBASE_SA_JSON) return json({ ok: false, reason: 'not-configured' }, 501);
      const rlKey = `rlr:${email}`;
      const sends = parseInt((await env.OTP_KV.get(rlKey)) ?? '0', 10);
      if (sends >= SENDS_PER_HOUR) return json({ ok: false, reason: 'rate' }, 429);
      await env.OTP_KV.put(rlKey, String(sends + 1), { expirationTtl: 3600 });

      let oobCode = null;
      try {
        const token = await getAccessToken(env);
        const res = await fetch('https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({ requestType: 'PASSWORD_RESET', email, returnOobLink: true }),
        });
        const data = await res.json();
        if (data?.oobLink) oobCode = new URL(data.oobLink).searchParams.get('oobCode');
        else if (!/EMAIL_NOT_FOUND/i.test(JSON.stringify(data))) throw new Error(JSON.stringify(data).slice(0, 150));
      } catch (e) {
        console.log('reset mint failed', String(e));
        return json({ ok: false, reason: 'send-failed' }, 502);
      }

      // Unknown emails get a generic ok (no account enumeration, no email).
      if (!oobCode) return json({ ok: true });

      const code = String(Math.floor(100000 + Math.random() * 900000));
      const record = { hash: await sha256(`${email}:${code}`), attemptsLeft: MAX_ATTEMPTS, oobCode };
      await env.OTP_KV.put(`reset:${email}`, JSON.stringify(record), { expirationTtl: CODE_TTL_SECONDS });
      try {
        await sendViaBrevo(env, email, code, 'reset', `${url.protocol}//${url.host}/logo-v2.png`);
      } catch (e) {
        await env.OTP_KV.delete(`reset:${email}`);
        console.log('reset send failed', String(e));
        return json({ ok: false, reason: 'send-failed' }, 502);
      }
      return json({ ok: true });
    }

    if (url.pathname === '/reset-verify') {
      const code = String(body.code ?? '').trim();
      if (!/^\d{6}$/.test(code)) return json({ ok: false, reason: 'That code is not right.' });
      const raw = await env.OTP_KV.get(`reset:${email}`);
      if (!raw) return json({ ok: false, reason: 'That code expired. Request a new one.' });
      const record = JSON.parse(raw);
      if (record.attemptsLeft <= 0) {
        await env.OTP_KV.delete(`reset:${email}`);
        return json({ ok: false, reason: 'Too many attempts. Request a new code.' });
      }
      const match = record.hash === (await sha256(`${email}:${code}`));
      if (!match) {
        record.attemptsLeft -= 1;
        await env.OTP_KV.put(`reset:${email}`, JSON.stringify(record), { expirationTtl: CODE_TTL_SECONDS });
        return json({ ok: false, reason: `That code is not right. ${record.attemptsLeft} tries left.` });
      }
      await env.OTP_KV.delete(`reset:${email}`);
      return json({ ok: true, oobCode: record.oobCode });
    }

    if (url.pathname === '/verify-otp') {
      const code = String(body.code ?? '').trim();
      if (!/^\d{6}$/.test(code)) return json({ ok: false, reason: 'That code is not right.' });

      const raw = await env.OTP_KV.get(`otp:${email}`);
      if (!raw) return json({ ok: false, reason: 'That code expired. Request a new one.' });
      const record = JSON.parse(raw);

      if (record.attemptsLeft <= 0) {
        await env.OTP_KV.delete(`otp:${email}`);
        return json({ ok: false, reason: 'Too many attempts. Request a new code.' });
      }

      const match = record.hash === (await sha256(`${email}:${code}`));
      if (!match) {
        record.attemptsLeft -= 1;
        await env.OTP_KV.put(`otp:${email}`, JSON.stringify(record), { expirationTtl: CODE_TTL_SECONDS });
        return json({ ok: false, reason: `That code is not right. ${record.attemptsLeft} tries left.` });
      }

      await env.OTP_KV.delete(`otp:${email}`);
      return json({ ok: true });
    }

    return json({ ok: false, reason: 'Not found' }, 404);
  },
};
