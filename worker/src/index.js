...
function splitShareHtml(senderName, p, logoUrl) {
  return emailShell(senderName, `
    <tr><td style="padding:24px 30px 8px;font-family:Arial,Helvetica,sans-serif;">
      ...
      <div style="color:#6C757D;font-size:14px;line-height:1.6;margin-top:8px;">Hi ${p.name}, ${p.payerName} paid this one and ${p.userName} split it ${p.headcount} ways.</div>
    </td></tr>
    <tr><td style="padding:14px 30px 4px;">
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#F1F6F2;border:1px solid #CFE3D6;border-radius:14px;">
        <tr><td style="padding:16px 18px;font-family:Arial,Helvetica,sans-serif;">
          <div style="color:#6C757D;font-size:13px;line-height:1.7;">${p.totalFmt} &divide; ${p.headcount} people</div>
...
function splitManageHtml(senderName, r, link, logoUrl) {
  return emailShell(senderName, `
    <tr><td style="padding:24px 30px 8px;font-family:Arial,Helvetica,sans-serif;">
      ...
      <div style="color:#6C757D;font-size:14px;line-height:1.6;margin-top:8px;">${r.userName} split it ${r.headcount} ways: ${r.totalFmt} total, ${r.shareFmt} each.</div>
    </td></tr>
...
async function handleSplitRoutes(request, env, url) {
  ...
  if (url.pathname === '/split-create' && request.method === 'POST') {
    ...
    const record = {
      ...
      my: { included: !!body.includeUser, label: clean(body.userLabel, 60) || 'the SaveCents user', paid: false },
    };
    ...
    const row = (label, paid, hidden) => `
      <form method="post" style="margin:0;">${hidden}
        <button type="submit" style="width:100%;display:flex;align-items:center;gap:12px;text-align:left;background:${paid ? '#F1F6F2' : '#fff'};border:1px solid ${paid ? '#CFE3D6' : '#E3E6E8'};border-radius:14px;padding:14px 16px;margin-bottom:10px;font-size:16px;font-family:inherit;color:#1A1D20;">
          ...
          <span style="font-weight:700;color:${paid ? '#6C757D' : '#165B33'};">${esc(r.shareFmt)}</span>
        </button>
      </form>`;
...
<body style="font-family:-apple-system,Segoe UI,Arial,sans-serif;background:#FAF9F6;color:#1A1D20;margin:0;padding:28px 20px;">
  <div style="max-width:440px;margin:0 auto;">
    ...
    <div style="color:#6C757D;font-size:14px;margin-bottom:18px;">${esc(r.totalFmt)} \u00f7 ${r.headcount} = <b style="color:#165B33;">${esc(r.shareFmt)} each</b> \u00b7 ${doneCount}/${totalCount} settled</div>
    ${r.my.included ? row(r.my.label, r.my.paid, '<input type="hidden" name="mine" value="1">') : ''}
    ${r.people.map((pp) => row(pp.name, pp.paid, `<input type="hidden" name="pid" value="${esc(pp.id)}">`)).join('')}
    ...
    if (url.pathname === '/send-split') {
      ...
      const res = await fetch('https://api.brevo.com/v3/smtp/email', {
        ...
        body: JSON.stringify({
          ...
          textContent: text,
          htmlContent: splitShareHtml(env.SENDER_NAME, { name, userName, payerName, title, totalFmt, shareFmt, headcount }, `${url.protocol}//${url.host}/logo-v2.png`),
        }),
      });
      if (!res.ok) {
