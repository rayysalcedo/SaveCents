...
var validEmail = /* @__PURE__ */ __name((e) => typeof e === "string" && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e) && e.length < 200, "validEmail");
...
async function getAccessToken(env) {
  ...
  const b64u = /* @__PURE__ */ __name((str) => btoa(str).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, ""), "b64u");
...
function emailShell(senderName, inner, logoUrl) {
  const brand = logoUrl ? `<img src="${logoUrl}" width="164" style="display:block;border:0;margin:0 auto;" alt="${senderName}" />` : `<span style="display:inline-block;background:#165B33;color:#FFFFFF;font-family:Arial,Helvetica,sans-serif;font-weight:800;font-size:13px;letter-spacing:0.4px;border-radius:999px;padding:8px 16px;">&#162; ${senderName}</span>`;
  return `<!DOCTYPE html>
...
function emailHtml(code, senderName, title, sub, logoUrl) {
  const digits = String(code).split("").map(
    (d) => `<td style="width:44px;height:56px;background:#F1F6F2;border:1px solid #CFE3D6;border-radius:12px;text-align:center;font-size:26px;font-weight:800;color:#165B33;font-family:'SF Mono',Menlo,Consolas,monospace;">${d}</td><td style="width:8px;"></td>`
  ).join("");
...
function splitShareHtml(senderName, p, logoUrl) {
  return emailShell(senderName, `
    ...
    <tr><td style="padding:14px 30px 4px;">
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#F1F6F2;border:1px solid #CFE3D6;border-radius:14px;">
        <tr><td style="padding:16px 18px;font-family:Arial,Helvetica,sans-serif;">
          ...
          <div style="color:#165B33;font-size:26px;font-weight:800;margin-top:4px;">${p.shareFmt}</div>
...
__name(splitShareHtml, "splitShareHtml");
function splitManageHtml(senderName, r, link, logoUrl) {
  return emailShell(senderName, `
    ...
    <tr><td align="center" style="padding:18px 30px 6px;">
      <a href="${link}" style="display:inline-block;background:#165B33;color:#FFFFFF;font-family:Arial,Helvetica,sans-serif;font-weight:800;font-size:15px;text-decoration:none;border-radius:14px;padding:14px 34px;">Manage the split</a>
    </td></tr>
...
__name(splitManageHtml, "splitManageHtml");
function lendEmailHtml(senderName, p, logoUrl) {
  return emailShell(senderName, `
    ...
    <tr><td style="padding:14px 30px 4px;">
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#F1F6F2;border:1px solid #CFE3D6;border-radius:14px;">
        <tr><td style="padding:16px 18px;font-family:Arial,Helvetica,sans-serif;">
          <div style="color:#165B33;font-size:26px;font-weight:800;">${p.amountFmt}</div>
...
__name(lendEmailHtml, "lendEmailHtml");
var SPLIT_TTL = 15 * 24 * 3600;
async function handleSplitRoutes(request, env, url) {
  ...
  if (url.pathname === "/split-create" && request.method === "POST") {
    ...
    const clean = /* @__PURE__ */ __name((v, n) => String(v ?? "").replace(/[<>]/g, "").trim().slice(0, n), "clean");
    const record = {
      ...
      people: (Array.isArray(body.people) ? body.people : []).slice(0, 20).map((pp) => ({
        ...
        shareFmt: clean(pp.shareFmt, 30) || void 0
      })),
      my: {
        included: !!body.includeUser,
        label: clean(body.userLabel, 60) || "the SaveCents user",
        paid: false,
        shareFmt: clean(body.userShareFmt, 30) || void 0
      },
    ...
    if (validEmail(payerEmail)) {
      const mathBit = record.even === false ? `${record.totalFmt}, split by what each person got` : `${record.totalFmt}, split ${record.headcount} ways, ${record.shareFmt} each`;
...
${record.userName}, via ${env.SENDER_NAME}`;
  ...
  if (seg[0] === "split-set" && seg[1] && request.method === "POST") {
    ...
    if (!raw) return json({ ok: false, reason: "gone" }, 404);
    const r = JSON.parse(raw);
    if (body.mine !== void 0) r.my.paid = !!body.mine;
  ...
  if (seg[0] === "split" && seg[1]) {
    ...
    const row = /* @__PURE__ */ __name((label, paid, hidden, shareFmt) => `
      <form method="post" style="margin:0;">${hidden}
        <button type="submit" style="width:100%;display:flex;align-items:center;gap:12px;text-align:left;background:${paid ? "#F1F6F2" : "#fff"};border:1px solid ${paid ? "#CFE3D6" : "#E3E6E8"};border-radius:14px;padding:14px 16px;margin-bottom:10px;font-size:16px;font-family:inherit;color:#1A1D20;">
          <span style="display:inline-flex;width:22px;height:22px;">${paid ? '<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" fill="#165B33"/><path d="M8 12.5l2.6 2.6L16 9.7" stroke="#FFFFFF" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>' : '<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="#ADB5BD" stroke-width="2"/></svg>'}</span>
          <span style="flex:1;${paid ? "text-decoration:line-through;color:#6C757D;" : ""}">${esc(label)}</span>
          <span style="font-weight:700;color:${paid ? "#6C757D" : "#165B33"};">${esc(shareFmt || r.shareFmt)}</span>
        </button>
      </form>`, "row");
    ...
    const mathLine = r.even === false ? `${esc(r.totalFmt)} total \xB7 <b style="color:#165B33;">custom shares</b> \xB7 ${doneCount}/${totalCount} settled` : `${esc(r.totalFmt)} \xF7 ${r.headcount} = <b style="color:#165B33;">${esc(r.shareFmt)} each</b> \xB7 ${doneCount}/${totalCount} settled`;
...
<body style="font-family:-apple-system,Segoe UI,Arial,sans-serif;background:#FAF9F6;color:#1A1D20;margin:0;padding:28px 20px;">
...
__name(handleSplitRoutes, "handleSplitRoutes");
function reportEmailHtml(senderName, monthLabel, preparedFor, stats, links, logoUrl) {
  const fmt = /* @__PURE__ */ __name((n) => "\u20B1" + Number(n).toLocaleString("en-PH", { maximumFractionDigits: 2 }), "fmt");
  const statCell = /* @__PURE__ */ __name((label, value, color) => `
  ...
  const btn = /* @__PURE__ */ __name((href, label, solid) => `
    <a href="${href}" style="display:inline-block;font-family:Arial,Helvetica,sans-serif;font-weight:800;font-size:14px;text-decoration:none;border-radius:12px;padding:13px 22px;margin-right:10px;${solid ? "background:#165B33;color:#FFFFFF;" : "background:#FFFFFF;color:#165B33;border:1px solid #CFE3D6;"}">${label}</a>`, "btn");
  return emailShell(senderName, `
...
async function sendViaBrevo(env, toEmail, code, kind, logoUrl) {
  ...
  const sub = kind === "reset" ? `Enter this code in the ${env.SENDER_NAME} app to set a new password. It works for the next 10 minutes.` : `Enter this code in the ${env.SENDER_NAME} app to continue. It works for the next 10 minutes.`;
  const res = await fetch("https://api.brevo.com/v3/smtp/email", {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      ...
      to: [{ email: toEmail }],
      subject: kind === "reset" ? `${code} is your ${env.SENDER_NAME} password reset code` : `${code} is your ${env.SENDER_NAME} code`,
...
var index_default = {
  async fetch(request, env) {
    ...
    if (url.pathname === "/send-split") {
      ...
      const mathBit = even ? `${totalFmt}, split ${headcount} ways` : `${totalFmt}, split by what each person got`;
      const text = `Hi ${name},
...
${userName}, via ${env.SENDER_NAME}`;
    ...
    if (url.pathname === "/send-report") {
      ...
      if (csvB64.length > 4e6 || pdfB64.length > 12e6) return json({ ok: false, reason: "too-big" }, 413);
