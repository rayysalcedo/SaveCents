...
export interface CentsSubAction {
  ...
  toAccountName?: string; // v5.48 MoveFunds: the DESTINATION account
...
export interface CentsResult {
  ...
  toAccountName?: string; // v5.48: MoveFunds destination ('' if none)
...
const INTENT_ENUM = [
  ...
  'RemoveCategory', 'UpdateBudget', 'RemoveTransaction', 'UpdateTransaction', 'SetBudgetDue', 'ShowTransactions', 'MoveFunds', 'CategoryMismatch', 'Unknown',
];
...
Valid intents: "LogTransaction", "PrePurchaseCheck", "AddCategory", "RemoveCategory", "UpdateBudget", "AddIncome", "AddGoal", "AddToGoal", "WithdrawFromGoal", "AddAccount", "SetAccountBalance", "RemoveTransaction", "UpdateTransaction", "SetBudgetDue", "ShowTransactions", "MoveFunds", "CategoryMismatch", "Unknown".
...
- For MoveFunds (moving money BETWEEN the user's own accounts: "move 500 from GCash to BPI", "transfer 2k to my savings", "ilipat mo yung 1,000 sa Maya"): 'amount', 'accountName' = the SOURCE, 'toAccountName' = the DESTINATION. This is NOT an expense and NOT income - money just changes pockets. If either side is missing, ask which account in ONE short question. Moving TO a credit card counts as paying it down.
